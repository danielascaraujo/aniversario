...
const eventDate = new Date("Jul 12, 2025 20:00:00");
...
const countTheTime = setInterval(function() {
    ...
    document.getElementById('countdown').innerHTML = `Que começa em ${daysUntilTheEvent}d ${hoursUntilTheEvent}h ${minutesUntilTheEvent}m ${secondsUntilTheEvent}s`;

    if (timeUntilTheEvent < 0) {
        clearInterval(countTheTime);
        document.getElementById('countdown').innerHTML = 'Que já acabou :('
    }
}, 1000);